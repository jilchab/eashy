...
impl Command {
    ...
    pub fn has_subcommands(&self) -> bool {
        matches!(self.children, Children::Subcmds(_))
    }
    ...
    pub fn get_usage_string(&self) -> String {
        ...
        for arg in &self.arguments {
            let display_name = if let Some(option) = &arg.option {
                format!("--{}", arg.name)
            } else {
                arg.name.clone()
            };
            if arg.prefix == ArgPrefix::None {
                args.push(display_name);
            } else if arg.prefix == ArgPrefix::ZeroMore {
                args.push(format!("[{0} ...]", display_name));
            } else if arg.prefix == ArgPrefix::OneMore {
                args.push(format!("{0} [{0} ...]", display_name));
            } else if arg.prefix == ArgPrefix::ZeroOne {
                args.push(format!("[{0}]", display_name));
            }
        }
    ...
    pub fn get_help_string(&self) -> String {
        ...
        if !opt_args.is_empty() {
            help_string.push_str(&format!("\n{TITLE}Optional arguments:{RESET}\n"));
            let mut width = 2;
            for arg in &opt_args {
                width = std::cmp::min(20, std::cmp::max(width, arg.name.len())) - 2;
            }
            for arg in opt_args {
                help_string.push_str(&format!("  {COMMAND}--{:width$}{RESET}  {}\n", arg.name, arg.help));
            }
